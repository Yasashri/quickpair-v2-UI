...
function Register() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || Object.values(err.response?.data?.errors || {}).flat().join(' ') || 'Unable to register.');
    }
  };
  ...
  return (
    <section className="page-card form-card">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <label>
          Confirm password
          <input value={form.password_confirmation} onChange={(e) => setForm({ ...form, password_confirmation: e.target.value })} type="password" minLength={8} required />
        </label>
